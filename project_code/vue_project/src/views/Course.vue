      ...
      <el-table-column label="操作"  width="400" align="center">
        <template slot-scope="scope">
          <el-button type="primary" @click="selectCourse(scope.row)" plain
                     v-if="user.role === 'STUDENT_CLINICAL' && (scope.row.property === '通识教育选修课' ||scope.row.property === '专业选修课')">选课 <i class="el-icon-edit"></i></el-button>
...
export default {
  ...
  methods:{
    selectCourse(row){
      this.request.post('/course/studentCourse/' + this.user.id + '/' + row.id).then(res => {
        if(res.code === '200'){
